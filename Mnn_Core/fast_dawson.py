...
class Dawson1:
    ...
    def int_fast(self, x):
        ...
        y[region2] = Chebyshev.chebval_no_transform(-np.abs(x[region2]), self.coef.cheb_G_neg, xmin = self.coef.cheb_xmin_for_G, xmax = 0, num_subdiv = self.coef.div, degree_cheb = self.coef.deg)        
...
class Dawson2:
    def __init__(self, N=30):
        ...
        self.coef.cheb_neg = Chebyshev.chebfit_neg(self.brute_force, num_subdiv = self.coef.div, degree_cheb = self.coef.deg)
        self.coef.cheb_H_neg = Chebyshev.chebfit_neg(self.int_exact, num_subdiv = self.coef.div, degree_cheb = self.coef.deg)        
        self.coef.cheb_H_pos = Chebyshev.chebfit_no_transform(lambda x: self.int_exact(x)/np.exp(2*x*x), xmin = 0, xmax = self.coef.cheb_xmas_for_H, num_subdiv = self.coef.div_pos, degree_cheb = self.coef.deg_pos)
    ...
    def dawson2(self,x):
        ...
        y[region2] = Chebyshev.chebval_neg(-np.abs(x[region2]), self.coef.cheb_neg, num_subdiv = self.coef.div, degree_cheb = self.coef.deg)
    ...
    def int_fast(self, x):
        ...
        y[region2_neg] = Chebyshev.chebval_neg(x[region2_neg], self.coef.cheb_H_neg, num_subdiv = self.coef.div, degree_cheb = self.coef.deg)
        y[region2_pos] = np.exp(2*x[region2_pos]*x[region2_pos])*Chebyshev.chebval_no_transform(x[region2_pos], self.coef.cheb_H_pos,  xmin = 0, xmax = self.coef.cheb_xmas_for_H, num_subdiv = self.coef.div_pos, degree_cheb = self.coef.deg_pos)
        return y
